...
String allSurahToJson(List<Map<String, AllAyat>> data) =>
    json.encode(List<dynamic>.from(data.map((x) =>
        Map.from(x).map((k, v) => MapEntry<String, dynamic>(k, v.toJson())))));

allSurahFromJson(String str) =>
    List<Map<String, AllAyat>>.from(json.decode(str).map((x) => Map.from(x)
        .map((k, v) => MapEntry<String, AllAyat>(k, AllAyat.fromJson(v)))));
...
class AllAyat {
  ...
  factory AllAyat.fromJson(Map<String, dynamic> json) => AllAyat(
